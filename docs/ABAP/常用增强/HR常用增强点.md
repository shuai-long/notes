...
这份 SAP Note 3125414 文档的核心内容是关于如何在 SAP HR 系统中为时间管理信息类型（编号 2000-2999）启用存在性图标（绿色对勾）的显示。以下是总结要点：

1. 问题描述（症状）：

- ∙标准系统行为： 在 PA30 等人事事务中，系统默认会为 HCM 主数据信息类型显示一个绿色对勾图标，表示该信息类型对选定的员工存在有效数据记录。
- ∙例外情况**：** 此图标默认不显示 给时间管理信息类型（编号 2000-2999），因为它们属于事务数据，系统检查逻辑不同。
- ∙特例： 信息类型 0007, 0080, 0081, 0082, 0416 是例外，它们像主数据信息类型一样会显示图标。

2. 解决方案演进：

- ∙过去（修改方式）： 需要通过 SAP Note 713327 提供的修改指令来实现，但这种方式在导入新的支持包后可能需要重新调整。
- ∙现在（无修改增强）： SAP 提供了一个隐式增强（Modification-free）选项，更加稳定，无需随支持包升级而重新实施。

3. 当前解决方案：

- ∙核心方法： 使用类 `CL_PT_TIME_UTIL`中的新方法 `USE_ICON_SPACE`。
- ∙实现方式： 通过隐式增强，将该方法的返回值 `RV_ICON_SPACE`从 `ABAP_TRUE`（标准行为，不显示图标）改为 `ABAP_FALSE`（为新行为，允许显示图标）。
- ∙增强功能：∙可以普遍为所有 2000-2999 范围的信息类型启用图标显示。∙可以利用参数 `IV_INFTY` 将图标显示限制到特定的个别信息类型。∙还可以根据事务代码（如 `SY-TCODE` ）进行更精细的控制。

4. 影响范围：

- ∙受影响的事务代码： PA20, PA30, PA51, PA53, PA61, PA62, PA63。
- ∙适用的软件组件和版本： SAP HR Reused Functionality，版本 600, 604, 608。

**总结：** 该文档说明了一种无需修改标准代码即可为时间管理信息类型启用存在性图标显示的方法，通过使用一个特定的类方法并进行隐式增强来实现，提高了解决方案的稳定性和可维护性。
