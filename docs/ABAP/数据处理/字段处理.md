...
* **小数转日期**
  ...
  ```abap
  class-methods conv_decimal_to_time
    importing
      value(iv_input)  type any
    returning
      value(rv_output) type tims .
  ```

  ```abap
  method conv_decimal_to_time.
    data: lv_num2       type n length 2,
          lv_p17        type p decimals 13,
          lv_fltp_value type qsollwerte,
          lv_char_field type qsollwertc.
  
    lv_fltp_value = iv_input.
  
    call function 'QSS0_FLTP_TO_CHAR_CONVERSION'
      exporting
        i_number_of_digits = 13
        i_fltp_value       = lv_fltp_value
      importing
        e_char_field       = lv_char_field.
  
    condense lv_char_field no-gaps.
    replace all occurrences of ',' in lv_char_field with '.'.
    
    "以下数据会有精度丢失问题，60秒不会舍入
    lv_p17 = lv_char_field * 24.
    lv_num2 = trunc( lv_p17 ).
    rv_output+0(2) = lv_num2.   "时
    lv_p17 = ( lv_p17 - lv_num2 ) * 60.
    lv_num2 = trunc( lv_p17 ).
    rv_output+2(2) = lv_num2.   "分
    lv_p17 = ( lv_p17 - lv_num2 ) * 60.
    lv_num2 = lv_p17.
    rv_output+4(2) = lv_num2.   "秒
  endmethod.
  ```
  
  ```abap
  method conv_decimal_to_time.   
     data:  lv_p17        type p length 16 decimals 13,
            lv_p17_int    type i,
            lv_fltp_value type qsollwerte,
            lv_char_field type qsollwertc.
      ...
      condense lv_char_field no-gaps.
      replace all occurrences of ',' in lv_char_field with '.'.
        lv_p17 = iv_input * 24 * 60 * 60 .
        lv_p17_int = lv_p17. "执行四舍五入
        rv_output = trunc( lv_p17_int ). 
  endmethod.
  ```
...
- 科学计数法转小数
  ...
  ```abap
  data: lv_fltp_value type qsollwerte,
        lv_char_field type qsollwertc.
