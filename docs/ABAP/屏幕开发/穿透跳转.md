...
```abap
method call_transaction_sm34.

  data: viewcluster_name   type vcl_name,
        maintenance_action type c value 'U'.

  viewcluster_name = iv_viewname.
...
```abap
data(lv_parameters) = |GODYNPRO-ACTION=A04;GODYNPRO-REFDOC=R02;GODYNPRO-MAT_DOC={ iv_mblnr };GODYNPRO-DOC_YEAR={ iv_gjahr } |.
