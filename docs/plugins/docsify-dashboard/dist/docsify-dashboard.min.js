...
function dashboardPlugin(t, e) {
  ...
  t.beforeEach(content => {
    // 1. 临时替换代码块内容（防止匹配到代码块中的注释）
    const codeBlocks = [];
    const placeholder = "___CODE_BLOCK_";
    content = content.replace(/```[\s\S]*?```/g, match => {
      codeBlocks.push(match);
      return `${placeholder}${codeBlocks.length - 1}___`; // 生成唯一占位符
    });
  
    // 2. 处理非代码块区域的 dashboard 注释
    const dashboardRegExp = /<!--\s*dashboard:\s*([^\s]+)(?:\s+(.+?))?\s*-->/g;
    content = content.replace(dashboardRegExp, (fullMatch, metadataUrl, paramsStr) => {
      try {
        // 参数解析逻辑保持不变
        const params = {};
        if (paramsStr) {
        ...
        const localNumTabContent = params.numTabContent 
          ? parseInt(params.numTabContent, 10) 
          : (n.numTabContent || 3);
        const localTheme = params.theme || r;
  
        let data = g(metadataUrl);
        if (s) data = d(data);
        return generateDashboard(data, localNumTabContent, localTheme);
      } catch (err) {
    ...
    // 3. 恢复被替换的代码块内容
    content = content.replace(new RegExp(`${placeholder}(\\d+)___`, "g"), (_, id) => {
  ...
  // // 修改后的 beforeEach 处理，支持局部 numTabContent
  // t.beforeEach(content => {
  //   const dashboardRegExp = /<!--\s*dashboard:\s*([^\s]+)(?:\s+(.+?))?\s*-->/g;
  //   let match;

  //   while ((match = dashboardRegExp.exec(content)) !== null) {
  //     const [fullMatch, metadataUrl, paramsStr] = match;
  //     try {
  //       // 解析参数（如 numTabContent=5）
  //       const params = {};
  //       if (paramsStr) {
  //         paramsStr.split(/\s+/).forEach(pair => {
  //           const [key, value] = pair.split('=');
  ...
  //       const localNumTabContent = params.numTabContent 
  //         ? parseInt(params.numTabContent, 10) 
  //         : (n.numTabContent || 3); // 优先用局部，否则全局或默认
  //       const localTheme = params.theme || r;

  //       // 加载数据并生成仪表盘
  //       let data = g(metadataUrl);
  //       if (s) data = d(data);
  //       const dashboardHTML = generateDashboard(data, localNumTabContent, localTheme);
